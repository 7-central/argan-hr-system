...
import type { SessionData, AdminSession } from './types';
import type { AdminRole } from '@prisma/client';

/**
 * Session duration: 4 hours (same as document-parser)
 */
...
export async function createAdminSession(admin: {
  ...
  const encryptedSession = encryptSession(sessionData, secret);
...
export async function validateAdminSession(): Promise<AdminSession | null> {
  ...
  try {
    const sessionData = decryptSession(sessionCookie.value, secret);
