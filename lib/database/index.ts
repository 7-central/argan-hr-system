...
import { config } from '@/lib/utils/system';
...
/**
 * Create a Prisma client with environment-specific configuration
 */
...
/**
 * Main database instance for production and development
 ...
 * Can be configured to use a separate test database
 */
export const testPrisma = globalForPrisma.testPrisma ?? createPrismaClient();

// Prevent multiple test instances in development
if (!config.app.isProduction) {
  globalForPrisma.testPrisma = testPrisma;
}

/**
 * Get the appropriate database instance based on NODE_ENV
 * - test: Returns test database instance
 * - development/production: Returns main database instance
 */
export function getDatabaseInstance(): PrismaClient {
  return config.app.isTest ? testPrisma : prisma;
}
...
export async function ensureConnection(): Promise<void> {
  ...
  if (!result.connected) {
    throw new DatabaseConnectionError(result.error || 'Database connection is not available');
  }
}
